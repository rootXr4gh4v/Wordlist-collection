...
wget -q https://raw.githubusercontent.com/haccer/subjack/master/fingerprints.json -O /root/tools/wordlists/subjack.json
if [ -s /root/tools/wordlists/subjack.json ]; then
    echo -e "${GR}SUCCESS${RT}"
else
    echo -e "${YW}FAILED${RT}"
fi
